...
public class ClassRegistrationUtils {
    ...
    public static void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    ArchSoupPvP.getInstance().getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), ArchSoupPvP.getInstance());
    ...
    public static Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = ArchSoupPvP.getInstance().getClass().getProtectionDomain().getCodeSource();
